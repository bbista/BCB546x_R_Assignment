...
maize.merge[!is.na(as.numeric(maize.merge$Chromosome)),] %>%  group_by(Chromosome) %>% arrange(as.numeric(Position)) %>%   do( write.csv( .,paste0("Maize_Chr_",unique(.$Chromosome),"_ascending.csv"), row.names = F))

teosinte.merge[!is.na(as.numeric(teosinte.merge$Chromosome)),] %>%  group_by(Chromosome) %>% arrange(as.numeric(Position)) %>%   do( write.csv( .,paste0("Teosinte_Chr_",unique(.$Chromosome),"_ascending.csv"), row.names = F))
...
#exporting files
maize[!is.na(as.numeric(maize$Chromosome)),] %>%  group_by(Chromosome) %>% arrange(desc(as.numeric(Position))) %>%   do( write.csv( .,paste0("Maize_Chr_",unique(.$Chromosome),"_descending.csv"), row.names = F))

teosinte[!is.na(as.numeric(teosinte$Chromosome)),] %>%  group_by(Chromosome) %>% arrange(desc(as.numeric(Position))) %>%   do( write.csv( .,paste0("Teosinte_Chr_",unique(.$Chromosome),"_descending.csv"), row.names = F))
